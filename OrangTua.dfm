object Form8: TForm8
  Left = 0
  Top = 0
  Caption = 'Orang Tua'
  ClientHeight = 526
  ClientWidth = 733
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 260
    Top = 80
    Width = 26
    Height = 19
    Caption = 'NIK'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Label2: TLabel
    Left = 245
    Top = 113
    Width = 45
    Height = 19
    Caption = 'NAMA'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Label6: TLabel
    Left = 290
    Top = 24
    Width = 182
    Height = 24
    Caption = 'DATA ORANG TUA'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -20
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label3: TLabel
    Left = 196
    Top = 144
    Width = 94
    Height = 19
    Caption = 'PENDIDIKAN'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Label4: TLabel
    Left = 218
    Top = 203
    Width = 68
    Height = 19
    Caption = 'TELEPON'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Label5: TLabel
    Left = 227
    Top = 236
    Width = 63
    Height = 19
    Caption = 'ALAMAT'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Label7: TLabel
    Left = 173
    Top = 270
    Width = 113
    Height = 19
    Caption = 'JENIS KELAMIN'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Label8: TLabel
    Left = 222
    Top = 301
    Width = 56
    Height = 19
    Caption = 'AGAMA'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Label9: TLabel
    Left = 204
    Top = 171
    Width = 86
    Height = 19
    Caption = 'PEKERJAAN'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Edit1: TEdit
    Left = 330
    Top = 77
    Width = 193
    Height = 27
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
  end
  object Edit2: TEdit
    Left = 330
    Top = 110
    Width = 193
    Height = 27
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 1
  end
  object BitBtn1: TBitBtn
    Left = 130
    Top = 343
    Width = 75
    Height = 25
    Caption = 'BARU'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    Glyph.Data = {
      
      C8C8C8C8C8C8C8C8009900C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8}
    ParentFont = False
    TabOrder = 2
    OnClick = BitBtn1Click
  end
  object BitBtn2: TBitBtn
    Left = 211
    Top = 343
    Width = 75
    Height = 25
    Caption = 'SIMPAN'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    Glyph.Data = {
      
      7DC9C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8}
    ParentFont = False
    TabOrder = 3
    OnClick = BitBtn2Click
  end
  object BitBtn3: TBitBtn
    Left = 290
    Top = 343
    Width = 75
    Height = 25
    Caption = 'UBAH'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    Glyph.Data = {
      
      C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8}
    ParentFont = False
    TabOrder = 4
    OnClick = BitBtn3Click
  end
  object BitBtn4: TBitBtn
    Left = 371
    Top = 343
    Width = 75
    Height = 25
    Caption = 'HAPUS'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    Glyph.Data = {
      
      C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8}
    ParentFont = False
    TabOrder = 5
    OnClick = BitBtn4Click
  end
  object BitBtn5: TBitBtn
    Left = 452
    Top = 343
    Width = 75
    Height = 25
    Caption = 'BATAL'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    Glyph.Data = {
      
      C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8C8}
    ParentFont = False
    TabOrder = 6
    OnClick = BitBtn5Click
  end
  object DBGrid1: TDBGrid
    Left = 8
    Top = 386
    Width = 717
    Height = 132
    DataSource = DataSource1
    TabOrder = 7
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    OnCellClick = DBGrid1CellClick
  end
  object RadioGroup1: TRadioGroup
    Left = 330
    Top = 266
    Width = 105
    Height = 31
    TabOrder = 8
  end
  object RadioButton1: TRadioButton
    Left = 343
    Top = 274
    Width = 28
    Height = 17
    Caption = 'L'
    TabOrder = 9
  end
  object RadioButton2: TRadioButton
    Left = 387
    Top = 274
    Width = 31
    Height = 17
    Caption = 'P'
    TabOrder = 10
  end
  object BitBtn6: TBitBtn
    Left = 533
    Top = 343
    Width = 75
    Height = 25
    Caption = 'LAPORAN'
    Glyph.Data = {
      
      9FC7249CC21086B0016D99003367C8C8C8C8C8C8C8C8C8C8C8C8}
    TabOrder = 11
    OnClick = BitBtn6Click
  end
  object ComboBox1: TComboBox
    Left = 330
    Top = 303
    Width = 193
    Height = 21
    TabOrder = 12
    Items.Strings = (
      'ISLAM'
      'KRISTEN'
      'HINDU'
      'BUDHA'
      'PROTESTAN')
  end
  object ComboBox2: TComboBox
    Left = 330
    Top = 146
    Width = 193
    Height = 21
    TabOrder = 13
    Items.Strings = (
      'S1'
      'S2'
      'S3')
  end
  object ComboBox3: TComboBox
    Left = 330
    Top = 173
    Width = 193
    Height = 21
    TabOrder = 14
    Items.Strings = (
      'Wiraswasta'
      'PNS'
      'POLRI/TNI')
  end
  object Edit3: TEdit
    Left = 330
    Top = 200
    Width = 193
    Height = 27
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 15
  end
  object Edit4: TEdit
    Left = 330
    Top = 233
    Width = 193
    Height = 27
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 16
  end
  object ZQuery1: TZQuery
    Connection = Form1.ZConnection1
    Active = True
    SQL.Strings = (
      'select * from ortu')
    Params = <>
    Left = 32
    Top = 24
    object ZQuery1id: TIntegerField
      FieldName = 'id'
      Required = True
    end
    object ZQuery1nik: TWideStringField
      FieldName = 'nik'
      Required = True
      Size = 16
    end
    object ZQuery1nama: TWideStringField
      FieldName = 'nama'
      Required = True
      Size = 50
    end
    object ZQuery1pendidikan: TWideStringField
      FieldName = 'pendidikan'
      Required = True
      Size = 30
    end
    object ZQuery1pekerjaan: TWideStringField
      FieldName = 'pekerjaan'
      Required = True
      Size = 30
    end
    object ZQuery1telepon: TWideStringField
      FieldName = 'telepon'
      Required = True
      Size = 13
    end
    object ZQuery1alamat: TWideStringField
      FieldName = 'alamat'
      Required = True
      Size = 30
    end
    object ZQuery1jk: TWideStringField
      FieldName = 'jk'
      Required = True
      Size = 1
    end
    object ZQuery1agama: TWideStringField
      FieldName = 'agama'
      Required = True
      Size = 15
    end
    object ZQuery1is_active: TWideStringField
      FieldName = 'is_active'
      Required = True
    end
  end
  object DataSource1: TDataSource
    DataSet = ZQuery1
    Left = 32
    Top = 96
  end
end
